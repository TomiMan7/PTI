...
private:
    ...
    void kiir (Csomopont * elem, std::ostream & os)
    {
        ...
        {
            ++melyseg;
		//kiir (elem->egyesGyermek (), os); //postorder kiiratas
		//kiir (elem->nullasGyermek (), os); //postorder kiirastas
            //kiir (elem->egyesGyermek (), os); // inorder kiiratas
            // ez a postorder bejáráshoz képest
            // 1-el nagyobb mélység, ezért -1
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;
            //kiir (elem->nullasGyermek (), os); // inorder kiiratas

		kiir (elem->egyesGyermek (), os); //preorder kiiratas
		kiir (elem->nullasGyermek (), os); //preorder kiiratas		
