...
private:
    ...
    {
        // Nem létező csomóponttal nem foglalkozunk... azaz ez a rekurzió leállítása
        if (elem != NULL)  // memoria szivargashoz ezt kell kommentezni
        {
...
int main (int argc, char *argv[])
{
    ...
    while (beFile.read ((char *) &b, sizeof (unsigned char))) //Ha nincs ujsor jel ">" akkor nem fogja megcsinalni a fat
        if (b == 0x0a)
            break;
    ...
    while (beFile.read ((char *) &b, sizeof (unsigned char)))
    {
        ...
        {			// újsor
            kommentben = false;
            continue;					//ezen karakterek beolvasasa eseten( ">" "ujsor" "N") nem csinal fat, ezeket ki kell kommentezni hogy megcsinalja
        }
    ...
    //operator<<(operator<<(kiFile, binFa),binFa);//operator<< kitolja a faljba a fat
   kiFile << binFa;// a << operator a kiFile-ba nyomja a binfat
